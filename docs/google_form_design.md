...
| 7 | 실내/실외 | `space_type` | 객관식 | 예 | `실내`, `실외`, `실내/실외` | `space_type` | 선택지를 CSV 값 그대로 저장 |
| 8 | 시설유형 | `category` | 객관식 | 예 | `관광지`, `영화/연극/공연`, `전시/기념관` | `category` | 현재 CSV 허용값만 제공. 새로운 유형은 Form에서 임의 입력하지 않음 |
| 9 | 입장료 여부 | `has_admission_fee` | 객관식 | 예 | `있음`, `없음` | `has_admission_fee` | 있음 → `TRUE`, 없음 → `FALSE` |
| 10 | 연령제한 여부 | `has_age_limit` | 객관식 | 예 | `있음`, `없음` | `has_age_limit` | 있음 → `TRUE`, 없음 → `FALSE` |
| 11 | 수유실 여부 | `nursing_room` | 객관식 | 예 | `있음`, `없음` | `nursing_room` | 있음 → `TRUE`, 없음 → `FALSE` |
| 12 | 유모차 대여 여부 | `stroller_rental` | 객관식 | 예 | `가능`, `불가` | `stroller_rental` | 가능 → `TRUE`, 불가 → `FALSE` |
| 13 | 주차 유형 | `parking` | 객관식 | 예 | `무료 주차`, `유료 주차`, `무료·유료 주차 모두 있음`, `주차 불가`, `잘 모르겠음` | `parking` | 앞의 네 선택지는 CSV 값으로 정규화. `잘 모르겠음`은 미확정 값으로 처리하며 신규는 승인 전에 관리자가 확인, 수정은 주차 변경 대상으로 선택하지 않았다면 기존 값 유지 |
