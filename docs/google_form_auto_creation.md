...
## 모르는 장소 정보를 선택 항목으로 바꾸기

이미 생성한 Form을 다시 만들 필요는 없다.

1. Apps Script의 `create_google_form.gs` 내용을 최신 코드로 교체하고 저장한다.
2. 상단 함수 목록에서 `applyOptionalPlaceInformationSettings`를 선택한다.
3. **실행**을 한 번 누른다.
4. 실행 로그에 `기본 정보 질문을 선택 항목으로 변경했습니다.`가 표시되는지 확인한다.

변경 후 실내/실외, 시설유형, 입장료, 연령제한, 수유실, 유모차 대여,
주차 질문은 모두 건너뛸 수 있다. `잘 모르겠음`과 시설유형의 `그 외`를
선택해도 검수용 `proposed_*` 값에는 빈칸으로 기록된다. CSV에 필요한 값은
관리자가 승인 전에 확인하여 입력한다.
